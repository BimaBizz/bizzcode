...
const Projects = (props) => {
  ...
  return (
    <section className='relative flex flex-col w-full max-w-5xl mx-auto p-4 justify-center items-center min-h-screen' typeof="BreadcrumbList">
        <h1 data-aos="fade-right" data-aos-duration="1500" className={`text-3xl font-bold self-start mb-10 ${props.class}`}>{props.title}</h1>
        <div className='grid md:grid-cols-2 gap-8 w-full p-4' vocab="https://schema.org/"  property="itemListElement" typeof="ListItem">
        {projects.map((project, index) => (
            <Link href={`/projects/${project.slug}`} className='w-full' key={index}  property="position" content={index+1} itemID={`https://admin.bizzcode.site/projects/${project.slug}`}>
                <div  className='flex flex-col space-y-3 w-full group' data-aos="fade-up" data-aos-duration={1000 +(200 * index)} property="item" typeof="WebPage">
                    ...
                    <h1 className='text-lg font-bold group-hover:text-indigo-500 transition-colors' itemProp="name">{project.name}</h1>
